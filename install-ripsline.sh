...
# ripsline Virtual Private Node Installer
...
# Verify we're running from the cloned repository
if [ ! -d "config" ] || [ ! -d "scripts" ]; then
    echo "❌ ERROR: config/ and scripts/ directories not found!"
    echo ""
    echo "This script must be run from the cloned repository:"
    echo ""
    echo "  git clone https://github.com/ripsline/virtual-private-node.git"
    echo "  cd virtual-private-node"
    echo "  sudo bash install-ripsline.sh"
    echo ""
    exit 1
fi

REPO_DIR=$(pwd)
echo "✅ Running from repository: $REPO_DIR"
echo ""
...
if [ -f "/root/ripsline/.installed" ]; then
    ...
    echo ""
    echo "If you need to:"
    echo "   • Update ripsline scripts: Use 'sudo /root/ripsline/scripts/update-ripsline.sh'"
...
echo "   Enter your domain for this Virtual Private Node"
echo "   (e.g., node.yourdomain.com or yourdomain.com)"
echo ""
read -p "Enter your domain: " BTCPAY_HOST
...
# Generate Lightning Terminal password
...
if [ -z "$RESOLVED_IP" ]; then
    ...
    echo "   Hostname: @ (for root) or subdomain name"
    echo "   Type: A"
    echo "   Points to: $DETECTED_IP"
    echo "   TTL: 3600"
    echo "3. Save and wait 10-30 minutes for propagation"
    echo "4. Check at: https://dnschecker.org"
...
# Copy config and scripts from repository
echo "📋 Copying configuration files and scripts..."
cp -r "$REPO_DIR/config" /root/ripsline/
cp -r "$REPO_DIR/scripts" /root/ripsline/

# Make scripts executable
chmod +x /root/ripsline/scripts/*.sh

echo "✅ Configuration and scripts ready"
echo ""
...
echo "IP: $DETECTED_IP" >> /root/ripsline/.installed
...
echo ""
echo "🔧 Useful Scripts:"
echo "   Install Haven Relay: sudo /root/ripsline/scripts/install-haven.sh"
echo "   Backup & SCB: sudo /root/ripsline/scripts/backup-and-scb.sh"
echo "   Fix LND Host: sudo /root/ripsline/scripts/fix-lnd-host.sh"
echo "   Update ripsline: sudo /root/ripsline/scripts/update-ripsline.sh"
echo ""
echo "🔄 After BTCPay updates, run:"
echo "   /root/ripsline/scripts/fix-lnd-host.sh"
echo ""
echo "📧 Need help? support@ripsline.com"
echo "📚 Docs: https://ripsline.com/docs"
echo ""
echo "================================================"
echo ""
